...
// --- AUSSCHUSS CODES (Eksiksiz Liste) ---
const purAusschussCodes = ["C102 CIM nicht voll", "C103 CIM beschädigt", "P101 Anfahrschrott PUR", "P102 PUR nicht voll", "P103 Schaum beschädigt", "P104 Schaumbild n.i.O.", "P105 Schaumhärtung n.i.O.", "P106 Einlegefehler", "6-2-01 Temperatur zu niedrig", "6-2-02 Schaum haftet nicht", "6-2-06 Lippe gerissen", "7-2-01 Nacharbeit", "Sonstige"];
...
// --- STÖRUNG CODES (Tüm Resimlerdeki 18+ Kodlar) ---
const purStoerungCodes = [
    "4-2-01 Werkzeug", "4-2-02 Ungepl. Instandhaltung Maschine", "4-2-03 POLY / ISO Überdruck", 
    ...
    "5-2-01 Keine Halbfertigteile bereitgestellt (Logistik)", "5-2-02 Keine Halbfertigteile von IM/CIM verfügbar", 
    "5-2-03 Anlernen von Mitarbeiter", "5-2-04 Keine Produktion (Wartezeit)", "5-2-05 Mitarbeiter Umbesetzung", 
    "5-2-06 Unterbesetzung", "5-2-07 Scanner / Drucker Probleme", "5-2-08 Kein Leergut bereitgestellt", 
    "5-2-09 Gasflasche wechseln", "5-2-10 Poly / Iso leer", "Sonstige"
];

const imStoerungCodes = [
    "3-01 Werkzeugwechsel", "3-02 Materialumstellung", "4-1-01 Ungepl. Instandhaltung Werkzeug", 
    "4-1-02 Ungepl. Instandhaltung Maschine", "4-1-03 Materialförderung gestört", "4-1-04 Probleme Dosiereinheit", 
    "4-1-05 Probleme Schließeinheit", "4-1-06 Probleme Teileentnahme", "4-1-07 Probleme Werkzeugheizung", 
    "4-1-08 Probleme Beflammprozess", "4-1-09 Entnahmeprobleme (Compound)", "5-1-01 Materialmangel", 
    "5-1-02 Anlernen", "5-1-03 Wartezeit", "5-1-04 Umbesetzung", "Sonstige"
];

const comStoerungCodes = [
    "3-01 Werkzeugwechsel", "3-02 Materialumstellung", "4-3-01 Lochplatte Messer schleifen", 
    "4-3-02 Ungepl. Instandhaltung Maschine", "4-3-03 Gepl. Instandhaltung Maschine", "4-3-04 Silowechsel", 
    "5-3-01 Materialmangel", "5-3-02 Anlernen", "5-3-03 Wartezeit", "5-3-04 Umbesetzung", "5-3-05 Feueralarm", "Sonstige"
];
...
document.getElementById("addArtikelBtn").addEventListener("click", () => {
    ...
    if(isCOM) {
        html += `<div class="grid"><div><label>Artikel</label><input class="artBez" type="text"></div><div><label>Art.Nummer</label><input class="artNum" type="text"></div></div><label>Dauer (Min)</label><input class="artDauer" type="number" value="0">`;
    } else {
...
function addAusRow(btn, anlage) {
    ...
    row.innerHTML = `<div style="flex:3; display:flex; flex-direction:column;"><select class="aCode" onchange="toggleCustomInput(this)">${list.map(c=>`<option value="${c}">${c}</option>`).join("")}</select><input type="text" class="customInput" placeholder="Grund beschreiben..." style="display:none; margin-top:5px;"></div><input type="number" class="aMenge" placeholder="Stk" style="flex:1; margin-left:10px; border:2px solid #cbd5e1; font-weight:bold; text-align:center;"><button type="button" class="row-delete-btn" style="margin-left:5px;" onclick="this.parentElement.remove(); validateAusschuss(document.querySelector('.artikel-box'))">X</button>`;
