...
addArtikelBtn.addEventListener("click", () => {
    ...
    box.querySelector(".addFehlerBtn").addEventListener("click", () => {
        ...
        row.innerHTML = `<div><input class="fehlerSelect" type="text" placeholder="Fehlergrund"></div><div><input class="fehlerMenge" type="number" placeholder="${unit}"></div>`;
...
async function speichern() {
    ...
    if (!anlageVal) return alert("❌ Fehler: Bitte wählen Sie eine Anlage aus!");
    if (workerBoxes.length === 0) return alert("❌ Fehler: Bitte mindestens einen Mitarbeiter hinzufügen!");
    if (artikelBoxes.length === 0) return alert("❌ Fehler: Bitte mindestens einen Artikel hinzufügen!");

    // 2. SÜRE KONTROLÜ
    let toplamDauer = 0;
    artikelBoxes.forEach(box => {
        toplamDauer += parseInt(box.querySelector(".artikelDauer").value || 0);
    });
    // Stoerung sürelerini artikel süresinden ayrı giriyorlarsa buraya eklemiyoruz (Senin isteğin üzerine sadece artikel sürelerini topluyoruz)

    if (toplamDauer !== sollDauer) {
        return alert(`⚠️ Zeit-Fehler!\nDie Summe der Artikel-Zeiten beträgt ${toplamDauer} Min.\nDie Soll-Zeit beträgt ${sollDauer} Min.\n\nBitte korrigieren Sie die Minuten!`);
    }
    ...
    artikelBoxes.forEach(box => {
        ...
        artikelText += `• ${bez} (${num}) | G: ${gut}${unit} | A: ${aus}${unit} | Zeit: ${dur} Min\n`;
